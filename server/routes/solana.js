...
router.post('/activate', async (req, res) => {
  ...
  if (!pkg || !hotspot) return res.status(400).json({ error: 'Unknown package or hotspot' });

  // Replay attack check
  const used = db.prepare('SELECT id FROM solana_payments WHERE tx_signature = ?').get(txSignature);
  if (used) return res.status(400).json({ error: 'Transaction already used' });
  ...
  if (!tx) return res.status(400).json({ error: 'Transaction not found on devnet' });
  ...
  db.prepare(`
    INSERT INTO solana_payments(tx_signature,wallet_address,hotspot_id,package_id,amount_sol,created_at)
    VALUES(?,?,?,?,?,datetime('now'))
  ...
  const result = db.prepare(`
    INSERT INTO sessions(ip,package_type,package_value,started_at,expires_at,tx_signature,active)
    VALUES(?,?,?,datetime('now'),?,?,1)
  `).run(userIP, pkg.type, pkg.value, expiresAt, txSignature);
